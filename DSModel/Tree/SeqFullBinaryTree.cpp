...
template<class T>
int SeqFullBinaryTree<T>::Size() const {
    return Size(0);
}

template<class T>
int SeqFullBinaryTree<T>::Size(int i) const {
    if (i >= currentSize)
        return 0;
    return 1 + Size(2 * i + 1) + Size(2 * i + 2);
}

template<class T>
int SeqFullBinaryTree<T>::SizeNonRecursionByLevelOrder() {
    queue<int> qu;
    int v = 0;
    qu.push(v);
    int size = 0;
...
template<class T>
int SeqFullBinaryTree<T>::SizeNonRecursionByPreOrder() {
    stack<int> st;
    int v = 0;
    int size = 0;
    while (v < currentSize || !st.empty()) {
...
template<class T>
int SeqFullBinaryTree<T>::LeavesSize() const {
    return LeavesSize(0);
}

template<class T>
int SeqFullBinaryTree<T>::LeavesSize(int i) const {
    if (i >= currentSize)
        return 0;
    if (2 * i + 1 >= currentSize && 2 * i + 2 >= currentSize)
        return 1;
    return LeavesSize(2 * i + 1) + LeavesSize(2 * i + 2);
}

template<class T>
int SeqFullBinaryTree<T>::LeavesSizeNonRecursionByLevelOrder() {
    queue<int> qu;
    int v = 0;
    qu.push(v);
    int num = 0;
    while (!qu.empty()) {
        v = qu.front();
        qu.pop();
        if (2 * v + 1 >= currentSize && 2 * v + 2 >= currentSize)
            num++;
        if (2 * v + 1 < currentSize)
            qu.push(2 * v + 1);
        if (2 * v + 2 < currentSize)
...
template<class T>
int SeqFullBinaryTree<T>::LeavesSizeNonRecursionByPreOrder() {
    stack<int> st;
    int v = 0;
    int num = 0;
    while (v < currentSize || !st.empty()) {
...
template<class T>
void SeqFullBinaryTree<T>::PrintBinTree() {
    PrintBinTree(0);
}

template<class T>
void SeqFullBinaryTree<T>::PrintBinTree(int i) {
    if (i < currentSize) {
        cout << elements[i] ;
//      2 * i + 1 < currentSize || 2 * i + 2 < currentSize简化了
